...
export const authEmpolyee = async (req,res,next) => {
    try{
        ...
        jwt.verify(token,'test', async (err,existingUser) => {
            ...
            //getting the usre 
            const user = existingUser.existingUser
            //checking for user role and if it is active
            if (user.type != 'employee' || user.state == 'false') return res.sendStatus(401);
            req.user = user;
...
export const auth = async (req,res,next) => {
    try{
        ...
        jwt.verify(token,'test', async (err,existingUser) => {
            ...
            const user = existingUser.existingUser
            //checking for user role and if it is active
            if ( user.state == 'false') return res.sendStatus(401);
            req.user = user;
